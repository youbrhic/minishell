...
/*   Updated: 2024/05/19 01:48:48 by youbrhic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char **get_newenv()
{
	...
	new_env[++i] = ft_strdup("PATH=/usr/gnu/bin:/usr/local/bin:/bin:/usr/bin:.");
	if (!new_env[i])
		return (free2d(&new_env, i), NULL);
    new_env[++i] = NULL;
    return (new_env);
...
void	ft_setenv(char *par, char *val, char ***env, int flag)
{
	...
	while ((*env)[++i])
	{
		if (!ft_strncmp((*env)[i], par, ft_strlen(par)) && (*env)[i][ft_strlen(par)] == '=')
		{
			if (flag)
				tmp = ft_strdup((*env)[i]);
			else
